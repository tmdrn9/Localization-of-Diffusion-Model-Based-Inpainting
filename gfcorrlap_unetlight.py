...
class GfDecoder(nn.Module):
    def __init__(self):
        ...
        self.Gf_extract = GFNet()
        self.softmax = nn.Softmax(dim=-1)
        self.up = nn.Upsample(scale_factor=32, mode='nearest')

        self.lap = nn.Parameter(torch.ones(3, 3, 3, 3), requires_grad=False)
        self.lap[:, :, 1, 1] = -8
        self.gelu = nn.GELU()

        diag = torch.eye(256).cuda()
        diag = torch.where(diag == 1., 0., 1.)
        self.diag = diag.unsqueeze(0)


    def forward(self, x):

        x_lap = F.conv2d(x, self.lap, padding=1)
        gf_feature = self.Gf_extract(x_lap) #batch,n_patch,embeddingDim
        corr = torch.matmul(gf_feature, gf_feature.permute(0,2,1)) #batch,n_patch,n_patch
        corr /= torch.sqrt(torch.tensor(1024))
        corr *= self.diag
        s = torch.sum(corr, dim=1)
        score = self.softmax(s)

        s_featuremap = score.reshape(-1,16,16)
        s_featuremap = s_featuremap.unsqueeze(1)
        out = self.up(s_featuremap)
        ...
        enc3_1 = self.enc5_1(pool2)
        dec3_1 = self.dec5_1(enc3_1)
...
class GFNet(nn.Module):
    ...
    def __init__(self, img_size=512, patch_size=32, in_chans=3, num_classes=1, embed_dim=1024, depth=8,
